...
class Library(customtkinter.CTk):
    ...
    def add_book_window(self):
        ...
        self.title_label = customtkinter.CTkLabel(self.add_window, text="Enter the details of your new book:")
        ...
        self.confirm_button = customtkinter.CTkButton(self.add_window, text="OK", command=self.add_confirmation)
        ...
        self.confirm_button.grid(row=4, column=0, padx=10, pady=10, sticky='w')
        self.cancel_button.grid(row=4, column=0, padx=10, pady=10, sticky='w')

    def add_confirmation(self, add_window, title, author, year, num_of_pages):
        title = self.title_entry.get()
        author = self.author_entry.get()
        year = int(self.year_entry.get())
        num_of_pages = int(self.num_of_pages_entry.get())
        if not (title and author and year and num_of_pages):
            print("Error", "Please enter all book details.")
        else:
            book_info = f"{title}, {author}, {year}, {num_of_pages}"
            self.text_widget.insert(book_info, END)
            add_window.destroy()
    ...
    def remove_book_window(self):
        ...
        self.remove_button = customtkinter.CTkButton(self.remove_window, text='Delete', command=self.delete_confirmation)
    ...
    def delete_confirmation(self, remove_window, remove_book):
        remove_book = self.remove_book_entry.get()

        for index in range(Library.text_widget.size()):
            if remove_book in Library.text_widget.get(index):
                self.text_widget.delete(index)
